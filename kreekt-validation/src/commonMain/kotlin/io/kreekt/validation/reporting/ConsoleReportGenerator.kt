...
internal class ConsoleReportGenerator(
    ...
    private fun StringBuilder.appendIssuesCounts(report: ProductionReadinessReport) {
        ...
        val criticalIssues = allCriteria.count { it.severity == Severity.CRITICAL && it.status != ValidationStatus.PASSED }
        val highIssues = allCriteria.count { it.severity == Severity.HIGH && it.status != ValidationStatus.PASSED }
        val mediumIssues = allCriteria.count { it.severity == Severity.MEDIUM && it.status != ValidationStatus.PASSED }
