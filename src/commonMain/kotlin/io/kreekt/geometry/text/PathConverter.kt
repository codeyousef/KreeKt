package io.kreekt.geometry.text
        ...
        for (command in path.commands) {
            when (command) {
                ...
                is PathCommand.BezierCurveTo -> {
                    ...
                    val curvePoints = CurveSubdivider.subdivideBezier(currentPoint, cp1, cp2, end, 12)
                    currentContour.addAll(curvePoints.drop(1)) // Skip first point (already added)
                    currentPoint = end
                }
...
object CurveSubdivider {
    ...
    fun subdivideQuadratic(start: Vector2, control: Vector2, end: Vector2, segments: Int): List<Vector2> {
    ...
    fun subdivideBezier(start: Vector2, cp1: Vector2, cp2: Vector2, end: Vector2, segments: Int): List<Vector2> {
        val points = mutableListOf<Vector2>()
