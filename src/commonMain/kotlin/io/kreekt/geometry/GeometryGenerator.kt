...
interface GeometryGenerator {
     ...
     */
    fun createBox(width: Float, height: Float, depth: Float, segments: IntArray = intArrayOf(1, 1, 1)): BufferGeometry
     ...
     */
    fun createPlane(width: Float, height: Float, widthSegments: Int = 1, heightSegments: Int = 1): BufferGeometry
...
class DefaultGeometryGenerator : GeometryGenerator {
    ...
    override fun createBox(width: Float, height: Float, depth: Float, segments: IntArray): BufferGeometry {
        ...
        buildPlane(vertices, normals, uvs, indices, 0, 1, 2, -1, -1, depth, height, width, depthSegments, heightSegments, vertexIndex)
        ...
        buildPlane(vertices, normals, uvs, indices, 2, 0, 1, 1, 1, width, height, depth, widthSegments, heightSegments, vertexIndex)
        ...
        buildPlane(vertices, normals, uvs, indices, 0, 2, 1, 1, 1, width, depth, height, widthSegments, depthSegments, vertexIndex)
    ...
    override fun createPlane(width: Float, height: Float, widthSegments: Int, heightSegments: Int): BufferGeometry {
        ...
        buildPlane(vertices, normals, uvs, indices, 0, 1, 2, 1, -1, width, height, 0f, widthSegments, heightSegments, 0)
    ...
    override fun createCylinder(
        ...
        for (y in 0..heightSegments) {
            ...
            for (x in 0..radialSegments) {
                ...
                val normal = Vector3(sinTheta, (radiusBottom - radiusTop) / height, cosTheta).normalized()
                normals.addAll(listOf(normal.x, normal.y, normal.z))
        ...
        if (!openEnded) {
            if (radiusTop > 0f) {
                generateCap(vertices, normals, uvs, indices, true, radiusTop, halfHeight, radialSegments, thetaStart, thetaLength, vertexIndex)
                vertexIndex = vertices.size / 3
            }
            if (radiusBottom > 0f) {
                generateCap(vertices, normals, uvs, indices, false, radiusBottom, -halfHeight, radialSegments, thetaStart, thetaLength, vertexIndex)
            }
        }
    ...
    override fun createTorus(radius: Float, tube: Float, radialSegments: Int, tubularSegments: Int, arc: Float): BufferGeometry {
        val vertices = mutableListOf<Float>()
        val normals = mutableListOf<Float>()
        val uvs = mutableListOf<Float>()
        val indices = mutableListOf<Int>()
    ...
    override fun createCone(
        ...
        return createCylinder(0f, radius, height, radialSegments, heightSegments, openEnded, thetaStart, thetaLength)
    }
