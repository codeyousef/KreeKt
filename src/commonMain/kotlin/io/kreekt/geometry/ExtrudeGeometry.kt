...
interface ExtrudeUVGenerator {
    fun generateSideUV(geometry: ExtrudeGeometry, vertices: List<Vector3>, indexA: Int, indexB: Int, indexC: Int, indexD: Int): List<Vector2>
    fun generateTopUV(geometry: ExtrudeGeometry, vertices: List<Vector3>, indexA: Int, indexB: Int, indexC: Int): List<Vector2>
    fun generateBottomUV(geometry: ExtrudeGeometry, vertices: List<Vector3>, indexA: Int, indexB: Int, indexC: Int): List<Vector2>
}
...
class DefaultExtrudeUVGenerator : ExtrudeUVGenerator {
    ...
    override fun generateBottomUV(geometry: ExtrudeGeometry, vertices: List<Vector3>, indexA: Int, indexB: Int, indexC: Int): List<Vector2> {
...
class ExtrudeGeometry(
    ...
    constructor(shape: Shape, options: ExtrudeOptions = ExtrudeOptions()) : this(listOf(shape), options)
    ...
    private fun generate() {
        ...
        for (shape in shapes) {
            ...
            generateCapFaces(triangulatedShape, layers, vertices, normals, uvs, indices, vertexCounter, true) // top
            vertexCounter = vertices.size
            generateCapFaces(triangulatedShape, layers, vertices, normals, uvs, indices, vertexCounter, false) // bottom
            vertexCounter = vertices.size
    ...
    private fun generateLayers(shapePoints: List<Vector2>, holes: List<List<Vector2>>): List<ExtrusionLayer> {
    ...
    private fun generateStraightLayers(shapePoints: List<Vector2>, holes: List<List<Vector2>>, layers: MutableList<ExtrusionLayer>) {
        ...
        for ((index, z) in layerPositions.withIndex()) {
            val scaleFactor = calculateBevelScale(index, layerPositions.size - 1, bevelEnabled, bevelSize, bevelOffset)
    ...
    private fun calculateBevelScale(index: Int, maxIndex: Int, bevelEnabled: Boolean, bevelSize: Float, bevelOffset: Float): Float {
    ...
    private fun generateSideFaces(
        ...
        for (layer in layers) {
            val layerPoints = if (layer.points.size == contourLength) {
                ...
                contour.mapIndexed { index, point ->
                    val scaledPoint = layer.points.getOrElse(index % layer.points.size) { layer.points.last() }
        ...
        for (layerIndex in 0 until layers.size - 1) {
            for (pointIndex in 0 until contourLength) {
                ...
                val sideUVs = uvGenerator.generateSideUV(this, vertices, a - vertexOffset, b - vertexOffset, c - vertexOffset, d - vertexOffset)
                uvs.addAll(sideUVs)
            }
        }
    }
    ...
    private fun generateCapFaces(
        ...
        for (triangle in triangulatedShape) {
            ...
            val capUVs = if (isTop) {
                uvGenerator.generateTopUV(this, vertices, a - vertexOffset, b - vertexOffset, c - vertexOffset)
            } else {
                uvGenerator.generateBottomUV(this, vertices, a - vertexOffset, b - vertexOffset, c - vertexOffset)
            }
            uvs.addAll(capUVs)
        }
    }
    ...
    private fun isEar(vertices: List<Vector2>, indices: List<Int>, prev: Int, current: Int, next: Int): Boolean {
        val a = vertices[prev]
