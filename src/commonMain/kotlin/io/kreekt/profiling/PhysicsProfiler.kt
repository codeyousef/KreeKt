...
object PhysicsProfiler {
    ...
    fun <T> profileNarrowPhase(collisionTests: Int, actualCollisions: Int, block: () -> T): T {
        ...
        PerformanceProfiler.recordCounter("physics.narrowPhase.collisions", actualCollisions.toLong())
    ...
    fun <T> profileConstraintSolver(constraintCount: Int, iterations: Int, block: () -> T): T {
        ...
        return PerformanceProfiler.measure("physics.constraintSolver", ProfileCategory.PHYSICS, block)
    }
    ...
    fun <T> profileCharacterController(block: () -> T): T {
        return PerformanceProfiler.measure("physics.characterController", ProfileCategory.PHYSICS, block)
    }
    ...
    fun <T> profileCollisionResponse(contactCount: Int, block: () -> T): T {
        ...
        return PerformanceProfiler.measure("physics.collisionResponse", ProfileCategory.PHYSICS, block)
    }
    ...
    fun analyzePhysicsComplexity(
        ...
        return PerformanceProfiler.measure("physics.analyzeComplexity", ProfileCategory.PHYSICS) {
            val averageContactsPerBody = if (bodyCount > 0) contactCount.toFloat() / bodyCount else 0f
            val averageConstraintsPerBody = if (bodyCount > 0) constraintCount.toFloat() / bodyCount else 0f
