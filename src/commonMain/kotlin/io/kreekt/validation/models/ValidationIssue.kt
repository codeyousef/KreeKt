...
data class CodeLocation(
    ...
    fun getModuleName(): String {
        ...
        // Look for module pattern like "kreekt-core", "kreekt-renderer"
        parts.forEach { part ->
            if (part.startsWith("kreekt-")) {
                return part
            }
        }

        // Try to extract from standard structure src/[platform]Main/kotlin/io/kreekt/[module]
        val kreektIndex = parts.indexOf("kreekt")
        if (kreektIndex >= 0 && kreektIndex < parts.size - 1) {
            return "kreekt-${parts[kreektIndex + 1]}"
        }
