...
abstract class BaseCollisionShape : CollisionShape {
    ...
    override fun localGetSupportingVertexWithoutMargin(direction: Vector3): Vector3 = localGetSupportingVertex(direction)
...
class DefaultCharacterController(
    ...
    var position: Vector3
        ...
        set(value) { transform = Matrix4.translation(value.x, value.y, value.z) }
...
class DefaultPhysicsEngine : PhysicsEngine {
    override val name: String = "Default KreeKt Physics Engine"
    ...
    override fun createCapsuleShape(radius: Float, height: Float): CapsuleShape = DefaultCapsuleShape(radius, height)
    ...
    override fun createCylinderShape(halfExtents: Vector3): CylinderShape {
        return object : BaseCollisionShape(), CylinderShape {
            ...
            override fun getVolume(): Float = PI.toFloat() * getRadius() * getRadius() * getHalfHeight() * 2f
            override fun calculateLocalInertia(mass: Float): Vector3 = Vector3(mass / 12f, mass / 2f, mass / 12f)
    ...
    override fun createConeShape(radius: Float, height: Float): ConeShape {
        return object : BaseCollisionShape(), ConeShape {
            ...
            override fun calculateLocalInertia(mass: Float): Vector3 = Vector3(mass / 20f, mass / 10f, mass / 20f)
            override fun calculateBoundingBox(): Box3 = Box3(Vector3(-radius, 0f, -radius), Vector3(radius, height, radius))
        }
    }
    ...
    override fun createCompoundShape(): CompoundShape {
        return object : BaseCollisionShape(), CompoundShape {
            ...
            override fun calculateLocalInertia(mass: Float): Vector3 = Vector3(mass / 6f, mass / 6f, mass / 6f)
            override fun addChildShape(transform: Matrix4, shape: CollisionShape): PhysicsResult<Unit> = PhysicsOperationResult.Success(Unit)
            override fun removeChildShape(shape: CollisionShape): PhysicsResult<Unit> = PhysicsOperationResult.Success(Unit)
            override fun removeChildShapeByIndex(index: Int): PhysicsResult<Unit> = PhysicsOperationResult.Success(Unit)
            override fun updateChildTransform(index: Int, transform: Matrix4): PhysicsResult<Unit> = PhysicsOperationResult.Success(Unit)
    ...
    override fun createPointToPointConstraint(bodyA: RigidBody, bodyB: RigidBody?, pivotA: Vector3, pivotB: Vector3): PointToPointConstraint {
    ...
    override fun createHingeConstraint(bodyA: RigidBody, bodyB: RigidBody?, pivotA: Vector3, pivotB: Vector3, axisA: Vector3, axisB: Vector3): HingeConstraint {
        return DefaultHingeConstraint("hinge_${++nextId}", bodyA, bodyB, pivotA, pivotB, axisA, axisB)
    }

    override fun createSliderConstraint(bodyA: RigidBody, bodyB: RigidBody?, frameA: Matrix4, frameB: Matrix4): SliderConstraint {
