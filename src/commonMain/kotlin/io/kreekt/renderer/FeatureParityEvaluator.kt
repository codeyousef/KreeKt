...
class FeatureParityEvaluator {
    ...
    fun evaluateSelection(
        ...
        return if (hasBlockedFeatures) {
            ...
            BackendSelection(
                ...
                parityMatrix = null,
                errorMessage = "Backend $backendId has blocked features: ${parityResults.filter { it.worstStatus() == ParityStatus.BLOCKED }.map { it.featureId }}"
            )
        } else {
    ...
    private fun evaluateFeatureParity(
        ...
        featuresToEvaluate.forEach { featureId ->
            ...
            results.add(FeatureParityMatrix(
                featureId = featureId,
