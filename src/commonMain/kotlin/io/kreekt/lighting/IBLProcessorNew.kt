...
class IBLProcessorImpl : IBLProcessor {
    ...
    suspend fun loadHDREnvironment(url: String): IBLResult<HDREnvironment> = withContext(dispatcher) {
        try {
            ...
            io.kreekt.lighting.ibl.IBLResult.Success(hdrData)
        } catch (e: Exception) {
            io.kreekt.lighting.ibl.IBLResult.Error("HDRLoadingFailed: $url, ${e.message}")
        }
    }
    ...
    suspend fun processEnvironment(
        ...
        try {
            ...
            io.kreekt.lighting.ibl.IBLResult.Success(
                IBLEnvironmentMaps(
                    environment = cubemap,
            ...
            io.kreekt.lighting.ibl.IBLResult.Error("ProcessingFailed: Failed to process environment, ${e.message}")
        }
    }
