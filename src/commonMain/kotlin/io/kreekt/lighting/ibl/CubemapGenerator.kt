...
internal object CubemapGenerator {
    ...
    suspend fun generateCubemapFromHDR(hdr: HDREnvironment, size: Int): IBLResult<CubeTexture> = withContext(Dispatchers.Default) {
        try {
            ...
            io.kreekt.lighting.ibl.IBLResult.Success(cubemap)
        } catch (e: Exception) {
            io.kreekt.lighting.ibl.IBLResult.Error("CubemapGenerationFailed: Failed to generate cubemap from HDR, ${e.message}")
        }
    }

    fun generateCubemapFace(hdr: HDREnvironment, size: Int, direction: Vector3, up: Vector3): FloatArray {
    ...
    private fun createEmptyCubemap(width: Int, height: Int, mipLevels: Int = 1): CubeTexture {
        ...
        return io.kreekt.texture.CubeTexture(
            size = width,
            format = io.kreekt.renderer.TextureFormat.RGBA32F,
