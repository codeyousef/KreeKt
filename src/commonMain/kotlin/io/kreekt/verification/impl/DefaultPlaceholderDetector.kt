package io.kreekt.verification.impl
...
class DefaultPlaceholderDetector(
            ...
            for (detectionPattern in configuration.patterns) {
                ...
                for (match: MatchResult in matches) {
                    val location = when (match) {
                        ...
                        else -> calculateLocationFromMultiline(filePath, content, match.range.first, match.value.length)
                    }
                    ...
                    patterns.add(
                        PlaceholderPattern(
                            ...
                            estimatedEffort = estimateEffortForPattern(detectionPattern.type, match.value),
    ...
    override suspend fun estimateImplementationEffort(
        placeholders: List<PlaceholderPattern>,
        historicalData: Map<PlaceholderType, io.kreekt.verification.model.Duration>?
    ): DetectionResult<Map<PlaceholderPattern, io.kreekt.verification.model.Duration>> {
        val estimates = placeholders.associateWith { placeholder ->
            historicalData?.get(placeholder.type) ?: estimateEffortForPattern(placeholder.type, placeholder.content)
        }
    ...
    private fun estimateEffortForPattern(
        ...
        content: String
    ): io.kreekt.verification.model.Duration {
        return when (type) {
            PlaceholderType.TODO -> io.kreekt.verification.model.Duration(
                2,
                io.kreekt.verification.model.Duration.TimeUnit.HOURS
            )

            PlaceholderType.FIXME -> io.kreekt.verification.model.Duration(
                4,
                io.kreekt.verification.model.Duration.TimeUnit.HOURS
            )

            PlaceholderType.STUB -> io.kreekt.verification.model.Duration(
                1,
                io.kreekt.verification.model.Duration.TimeUnit.DAYS
            )

            PlaceholderType.PLACEHOLDER -> io.kreekt.verification.model.Duration(
                4,
                io.kreekt.verification.model.Duration.TimeUnit.HOURS
            )

            PlaceholderType.WORKAROUND -> io.kreekt.verification.model.Duration(
                1,
                io.kreekt.verification.model.Duration.TimeUnit.DAYS
            )

            PlaceholderType.FOR_NOW -> io.kreekt.verification.model.Duration(
                8,
                io.kreekt.verification.model.Duration.TimeUnit.HOURS
            )

            PlaceholderType.IN_THE_MEANTIME -> io.kreekt.verification.model.Duration(
                8,
                io.kreekt.verification.model.Duration.TimeUnit.HOURS
            )

            PlaceholderType.REAL_IMPLEMENTATION -> io.kreekt.verification.model.Duration(
                3,
                io.kreekt.verification.model.Duration.TimeUnit.DAYS
            )

            PlaceholderType.NOT_IMPLEMENTED -> io.kreekt.verification.model.Duration(
                2,
                io.kreekt.verification.model.Duration.TimeUnit.DAYS
            )
        }
