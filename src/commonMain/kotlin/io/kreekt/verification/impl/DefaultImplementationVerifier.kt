...
class DefaultImplementationVerifier(
    ...
    override suspend fun generateImplementationTasks(
        ...
        for ((moduleType, moduleArtifacts) in artifactsByModule) {
            for (artifact in moduleArtifacts.sortedBy { it.priority }) {
                ...
                when (val detectionResult = placeholderDetector.detectPlaceholders(artifact.filePath)) {
                    is DetectionResult.Success -> {
                        val placeholders = detectionResult.data
    ...
    override suspend fun verifyImplementationComplete(
        ...
        return when (val detectionResult = placeholderDetector.detectPlaceholders(artifact.filePath)) {
            is DetectionResult.Success -> {
        ...
        return try {
            ...
            when (val scanResult = scanCodebase(rootPath, excludePatterns)) {
                is VerificationResult.Success -> {
                    ...
                    val totalArtifacts = artifacts.size
                    val completeArtifacts = artifacts.count { it.implementationStatus == ImplementationStatus.COMPLETE }
                    ...
                    val overallProgress = if (totalArtifacts > 0) completeArtifacts.toFloat() / totalArtifacts else 1.0f

                    val moduleProgress = artifacts.groupBy { it.moduleType }.mapValues { (_, moduleArtifacts) ->
                        val moduleComplete =
        ...
        for (gate in gates) {
            ...
            for (criteria in gate.criteria) {
                when (criteria.name) {
                    ...
                    "Test Coverage" -> {
                        val threshold = criteria.threshold.removeSuffix("%").toFloatOrNull()?.div(100f) ?: 0.8f
                    ...
                    "Constitutional Compliance" -> {
                        val nonCompliantArtifacts = artifacts.filter { !it.constitutionalCompliance }
    ...
    // Helper methods
    private suspend fun findKotlinFiles(rootPath: String, excludePatterns: List<String>): List<String> {
    ...
    private fun determinePriority(placeholders: List<PlaceholderPattern>, moduleType: ModuleType): Priority {
    ...
    private fun calculateTotalEffort(placeholders: List<PlaceholderPattern>): io.kreekt.verification.model.Duration {
        val totalHours = placeholders.sumOf { it.estimatedEffort.toHours() }
        return io.kreekt.verification.model.Duration(
            totalHours.toLong(),
    ...
    private fun generateTestRequirements(
        ...
        if (artifact.moduleType in setOf(ModuleType.RENDERER, ModuleType.ANIMATION, ModuleType.PHYSICS)) {
