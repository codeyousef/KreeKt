...
open class Path {
    ...
    sealed class PathSegment {
        ...
        data class BezierCurveTo(val control1: Vector2, val control2: Vector2, val point: Vector2) : PathSegment()
        data class ArcTo(
            val center: Vector2,
            val radius: Vector2,
        ...
        for (segment in segments) {
            when (segment) {
                ...
                is PathSegment.BezierCurveTo -> {
                    ...
                    for (i in 1..divisions) {
                        ...
                        val p = current.clone().multiplyScalar(oneMinusT * oneMinusT * oneMinusT)
                            .add(segment.control1.clone().multiplyScalar(3f * oneMinusT * oneMinusT * t))
                ...
                is PathSegment.ArcTo -> {
                    ...
                    for (i in 1..divisions) {
                        val t = i.toFloat() / divisions
                        val angle = segment.startAngle + (if (segment.clockwise) angleRange * t else -angleRange * t)
    ...
    open fun ellipse(
        ...
        segments.add(PathSegment.ArcTo(center, radius, aStartAngle + aRotation, aEndAngle + aRotation, aClockwise))
