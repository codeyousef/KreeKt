...
open class ShaderMaterial(
    ...
    fun validate(): ShaderValidationResult {
        ...
        if (vertexShader.isNotEmpty()) {
            issues.addAll(
                ShaderValidator.validateShaderSyntax(vertexShader, ShaderStage.VERTEX, shaderLanguage)
                    .map { "Vertex shader: $it" }
            )
        }
        if (fragmentShader.isNotEmpty()) {
            issues.addAll(
                ShaderValidator.validateShaderSyntax(fragmentShader, ShaderStage.FRAGMENT, shaderLanguage)
                    .map { "Fragment shader: $it" }
            )
        }
        if (computeShader.isNotEmpty()) {
            issues.addAll(
                ShaderValidator.validateShaderSyntax(computeShader, ShaderStage.COMPUTE, shaderLanguage)
