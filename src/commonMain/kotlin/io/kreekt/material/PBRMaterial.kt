...
class PBRMaterial(
    ...
    fun getClearcoatContribution(uv: Vector2): ClearcoatContribution {
        ...
        clearcoatMap?.let { texture ->
            intensity *= texture.sample(uv).r
        }

        clearcoatRoughnessMap?.let { texture ->
            roughness *= texture.sample(uv).g
        }
    ...
    fun getTransmissionContribution(uv: Vector2): TransmissionContribution {
        ...
        transmissionMap?.let { texture ->
            factor *= texture.sample(uv).r
        }

        thicknessMap?.let { texture ->
            thickness *= texture.sample(uv).g
        }
...
abstract class Material(
    ...
    companion object {
        private fun generateId(): String {
            return "material-${System.currentTimeMillis()}-${(Math.random() * 1000000).toInt()}"
        }
    }
}
...
data class Vector2Animation(
    ...
    fun getValueAtTime(time: Float): Vector2 {
        ...
        if (keyframes.size == 1) return keyframes[0].value.clone()
        ...
        if (lowerIndex == upperIndex) {
            return keyframes[lowerIndex].value.clone()
        }
        ...
        return lower.value.clone().lerp(upper.value, t)
    }
...
data class FloatAnimation(
    ...
    fun getValueAtTime(time: Float): Float {
        ...
        if (lowerIndex == upperIndex) {
            return keyframes[lowerIndex].value
        }
