...
class HierarchicalZBuffer(
    ...
    fun isOccluded(box: BoundingBox, viewProjection: Matrix4): Boolean {
        ...
        val minY = (screenBox.min.y * (height shr level)).toInt().coerceIn(0, (height shr level) - 1)
        val maxY = (screenBox.max.y * (height shr level)).toInt().coerceIn(0, (height shr level) - 1)
