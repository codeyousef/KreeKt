...
interface MorphTargetGeometry {
    ...
    fun addMorphTarget(
        ...
        val positionTargets = morphAttributes.getOrPut("position") { mutableListOf() }.toMutableList()
        positionTargets.add(position)
        morphAttributes["position"] = positionTargets

        // Add normal morph target if provided
        if (normal != null) {
            val normalTargets = morphAttributes.getOrPut("normal") { mutableListOf() }.toMutableList()
