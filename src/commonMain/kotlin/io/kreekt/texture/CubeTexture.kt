...
class CubeTexture(
    ...
    private fun sampleDirection(direction: Vector3): Color {
        ...
        val (face, u, v) = when {
            absX >= absY && absX >= absZ -> {
                if (direction.x > 0) {
                    Triple(CubeFace.POSITIVE_X, -direction.z / direction.x, -direction.y / direction.x)
                } else {
                    Triple(CubeFace.NEGATIVE_X, direction.z / direction.x, -direction.y / direction.x)
                }
            }

            absY >= absZ -> {
                if (direction.y > 0) {
                    Triple(CubeFace.POSITIVE_Y, direction.x / direction.y, direction.z / direction.y)
                } else {
                    Triple(CubeFace.NEGATIVE_Y, direction.x / direction.y, -direction.z / direction.y)
                }
            }

            else -> {
                if (direction.z > 0) {
                    Triple(CubeFace.POSITIVE_Z, direction.x / direction.z, -direction.y / direction.z)
                } else {
                    Triple(CubeFace.NEGATIVE_Z, -direction.x / direction.z, -direction.y / direction.z)
                }
            }
        }
