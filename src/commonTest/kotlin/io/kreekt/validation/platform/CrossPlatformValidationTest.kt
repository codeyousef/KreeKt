...
import io.kreekt.validation.GapSeverity
import io.kreekt.validation.GapType
import io.kreekt.validation.Platform
...
class CrossPlatformValidationTest {
    ...
    private val projectRoot = "/mnt/d/Projects/KMP/KreeKt"
    ...
    fun testAllPlatformSupport() = runTest {
        ...
        val requiredPlatforms = setOf(Platform.JVM, Platform.JS, Platform.ANDROID, Platform.IOS, Platform.NATIVE)

        requiredPlatforms.forEach { platform ->
            assertTrue(
                supportedPlatforms.contains(platform) || rendererAudit.missingPlatforms.contains(platform),
    ...
    fun testMultiplatformAPIConsistency() = runTest {
        ...
        criticalGaps.forEach { gap ->
            ...
            assertFalse(
                gap.module.contains("core", ignoreCase = true) || gap.module.contains("api", ignoreCase = true),
    ...
    fun testCrossPlatformTypesSafety() = runTest {
        ...
        val criticalTypeSafetyIssues = typeSafetyIssues.filter { it.severity == GapSeverity.CRITICAL }
    ...
    fun testPlatformSpecificOptimizations() = runTest {
        ...
        rendererAudit.rendererComponents.forEach { (platform, component) ->
            ...
            val criticalIssues = component.issues.filter { it.contains("critical", ignoreCase = true) }
            assertTrue(
