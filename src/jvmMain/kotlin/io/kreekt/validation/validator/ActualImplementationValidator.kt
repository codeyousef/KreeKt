...
class ActualImplementationValidator : ImplementationValidator {
            ...
            for (expectDeclaration in expectDeclarations) {
                ...
                if (expectInfo != null) {
                    ...
                    for (platform in platforms) {
                        val hasActual = hasActualImplementation(expectDeclaration, platform, sourceRoot)
                        ...
                        if (!hasActual) {
                            gaps.add(
                                ImplementationGap(
                                    ...
                                    severity = calculateGapSeverity(expectInfo.module, expectDeclaration),
            ...
            for ((lineIndex, line) in lines.withIndex()) {
                for (pattern in EXPECT_PATTERNS) {
                    ...
                    for (match in matches) {
                        ...
                        for (platform in platforms) {
                            val hasActual =
                                hasActualImplementation(expectDeclaration, platform, File(expectFilePath).parent)
    ...
    override suspend fun findExpectDeclarations(sourceRoot: String): List<String> = withContext(Dispatchers.IO) {
