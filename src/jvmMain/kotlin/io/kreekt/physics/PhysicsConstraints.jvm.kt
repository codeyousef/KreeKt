...
class JvmHingeConstraint(
    ...
    fun solveConstraint(deltaTime: Float) {
        ...
        // Apply motor
        if (enableAngularMotor) {
            val motorImpulse = (targetVelocity * deltaTime).coerceIn(-maxMotorImpulse, maxMotorImpulse)
    ...
    private fun applyAngularImpulse(impulse: Float) {
        val worldAxis =
            Matrix4().makeRotationFromQuaternion(bodyA.getWorldTransform().getRotation()).transformDirection(axisA)
...
class JvmSliderConstraint(
    ...
    override fun getLinearPos(): Float {
        ...
        val sliderAxis =
            Matrix4().makeRotationFromQuaternion(worldFrameA.getRotation()).transformDirection(Vector3.UNIT_X)
