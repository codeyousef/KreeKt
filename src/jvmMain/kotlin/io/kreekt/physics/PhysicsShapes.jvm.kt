...
abstract class JvmCollisionShape : CollisionShape {
    ...
    override fun getSurfaceArea(): Float = boundingBox.getSize().let { 2f * (it.x * it.y + it.y * it.z + it.z * it.x) }
    override fun isConvex(): Boolean = shapeType != ShapeType.TRIANGLE_MESH && shapeType != ShapeType.HEIGHTFIELD
...
class JvmSphereShape(override val radius: Float) : JvmCollisionShape(), SphereShape {
    ...
    override fun getVolume(): Float = (4f / 3f) * kotlin.math.PI.toFloat() * radius * radius * radius
...
class JvmCapsuleShape(
    ...
    init {
        ...
        _boundingBox = Box3(Vector3(-radius, -halfHeight, -radius), Vector3(radius, halfHeight, radius))
    }
...
class JvmTriangleMeshShape(
    ...
    override fun getTriangle(index: Int): Triangle =
        if (index < triangles.size) triangles[index] else Triangle(Vector3.ZERO, Vector3.ZERO, Vector3.ZERO)
...
class JvmHeightfieldShape(
    ...
    init {
        ...
        _boundingBox = Box3(Vector3(-halfWidth, minHeight, -halfDepth), Vector3(halfWidth, maxHeight, halfDepth))
    }
