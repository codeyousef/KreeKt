...
class JvmRigidBody(
    ...
    override fun applyImpulse(impulse: Vector3, relativePosition: Vector3): PhysicsResult<Unit> {
        ...
        if (mass > 0f) {
            ...
            if (relativePosition != Vector3.ZERO) {
                ...
                angularVelocity = angularVelocity.plus(_inverseInertia.times(torqueImpulse).times(angularFactor))
            }
        }
    ...
    fun integrateVelocities(deltaTime: Float) {
        if (bodyType == RigidBodyType.DYNAMIC && mass > 0f) {
            // Linear integration
            linearVelocity = linearVelocity.plus(totalForce.div(mass).times(deltaTime).times(linearFactor))

            // Angular integration
            angularVelocity =
                angularVelocity.plus(_inverseInertia.times(totalTorque).times(deltaTime).times(angularFactor))
    ...
    override fun jump(direction: Vector3) {
        if (canJump()) {
            ...
            if (direction != Vector3.UNIT_Y) {
                ...
                velocityForTimeInterval = velocityForTimeInterval.plus(horizontalDir.times(jumpSpeed * 0.5f))
            }
        }
    }
...
) : io.kreekt.physics.RaycastResult {
