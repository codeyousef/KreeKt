...
class BufferManager(private val gl: dynamic) {
    ...
    fun createCubeBuffers(name: String): BufferResult<Triple<ManagedBuffer, ManagedBuffer, ManagedBuffer>> {
        ...
        return when {
            vertexBuffer is BufferResult.Success &&
                ...
                BufferResult.Success(Triple(vertexBuffer.value, colorBuffer.value, indexBuffer.value))
            }
    ...
    fun drawArrays(
        ...
        return try {
            val drawCount = count ?: (arrayBuffer.elementCount / 3) // Assume 3 components per vertex
    ...
    fun getBufferStats(): Map<String, Any> {
        ...
        val elementBuffers = managedBuffers.values.count { it.type == BufferType.ELEMENT_ARRAY_BUFFER }
 ...
 */
fun createPositionColorAttributes(positionLocation: Int, colorLocation: Int): List<VertexAttribute> {
    return listOf(
        VertexAttribute(
